...
- [RMarkdown](https://rmarkdown.rstudio.com/) is "an authoring framework for data science" that facilitates literate programming and reproduceable research
 ...
 - Outut formats: PDF, HTML, Word, Shiny (for interactive web apps), others
...
<!-- This map code needs work... -->

<!-- --- -->

<!-- **Another Cool Viz: Maps** -->

<!-- ```{r fig.cap="Cumulative Case Percent",fig.width=4, fig.asp=0.9,comment=NA}  -->
...
<!-- COVID<-merge(COVID,PAmap,by=c("FIPS")) -->
<!-- COVID <- data.frame(COVID) -->

<!-- # Make variables & legend breaks: -->

<!-- COVID$CPct<-round(COVID$Case.Percent,1) -->
<!-- my_breaks = c(0.5,1,2,5,10,20) -->

<!-- COVID %>% -->
<!--   ggplot() + theme_void() + -->
<!--   geom_sf(mapping = aes(fill = Case.Percent), -->
<!--           color = gray(0), size = 0.05) + -->
<!--   scale_fill_gradientn(name="Cases (Percent)", -->
<!--                        breaks=my_breaks, labels=my_breaks, -->
<!--                        colours=c("darkgreen","yellow","red")) + -->
<!--   labs(title="FOO", -->
<!--        fill="Cases per 100 Population", -->
<!--        subtitle="     Source:https://github.com/nytimes/covid-19-data") + -->
<!--   geom_sf_text(aes(label=CPct,geometry=geometry), -->
<!--                fun.geometry=st_centroid,cex=2,col="black") -->
<!-- ``` -->
...
with(COVID[COVID$Urb=="Urban",],
     plot(Cumulative.Case.Rate,Total.Death.Rate,
